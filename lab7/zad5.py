...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            int vertex_number = abs((gl_VertexID % 6) * 2 - 5) + gl_VertexID / 6;
            mat4 someMatrix;
            ...
            someMatrix[3] = vec4((gl_InstanceID % 10 - 4) * 5, (gl_InstanceID / 10 - 4) * 5, 0.0, 1.0);
            transformedPosition[0] *= (gl_InstanceID * gl_InstanceID) % 7 + 1;
            transformedPosition[1] *= ((gl_InstanceID * gl_InstanceID) % 11 + 2) / 2;
            transformedPosition[2] *= (gl_InstanceID * gl_InstanceID) % 3 + 3;
            gl_Position = P_matrix * V_matrix * someMatrix * M_matrix  * transformedPosition;
            vertex_color = vec4(0.0);
            vertex_color[0] = step(1.0, (((15 * vertex_number + 3 * gl_InstanceID + 1) % 23 + 6) / 22.0));
            vertex_color[1] = step(1.0, (((15 * vertex_number + 2 * gl_InstanceID + 9) % 17 + 4) / 16.0));
            vertex_color[2] = step(1.0, (((15 * vertex_number + 7 * gl_InstanceID + 7) % 19 + 5) / 18.0));
            vertex_color[3] = input_color[3];
        }
    """
